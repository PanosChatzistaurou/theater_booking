...
const createReservation = async (req, res) => {
    ...
    if (!showtime_id || !seat_id) {
        return res.status(400).json({ error: 'showtime_id and seat_id are required' });
    }
    ...
    try {
        ...
        if (existing.length > 0) {
            ...
            if (resRecord.status === 'PENDING' || resRecord.status === 'CONFIRMED') {
                return res.status(409).json({ error: 'Seat is already reserved for this showtime' });
            }
...
const confirmReservation = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Failed to confirm reservation', details: err.message });
    } finally {
        if (conn) conn.release();
    }
};
    ...
    try {
        ...
        const rows = await conn.query(
            `SELECT
             ...
             JOIN theatre_seats ts ON ts.id        = r.seat_id
             WHERE r.user_id = ?
